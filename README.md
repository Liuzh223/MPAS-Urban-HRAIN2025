...
The project is distributed in three parts so that large input files and locally
generated model files are not mixed with the Git repository.

| Location | What it contains | What it is for |
|---|---|---|
| Git repository | PDFs, reference namelists, mesh-generation script, and 112-task graph partition | Instructions and case configuration |
| v1.0 Release | Ready 30 km-500 m grid and one GFS intermediate file | Beginner initialization path |
| Your MPAS installation and run directory | Executables, static datasets, vertical levels, static/initial files, and model output | Files installed or generated locally |
...
| HRAIN2025 case-specific source | Reproducing the case-author configuration, including the 10 km CU cutoff and the historical-result-compatible `ZT` setting | [`Liuzh223/HKUST-MPAS-Official`, branch `ntdk-grid-cutoff`](https://github.com/Liuzh223/HKUST-MPAS-Official/tree/ntdk-grid-cutoff) |

The standard `hkust-dev` branch is the default recommendation for learning and
new experiments. The HRAIN2025 case-specific source is fixed for this tutorial
at commit
...
released. This section is only for a future direct comparison with those
existing results; it is not required for learning MPAS or running a new
independent experiment.

Use the pinned HRAIN2025 case-specific source at
[`57b42fa1d7649117bbf28a69c47697ea8756d268`](https://github.com/Liuzh223/HKUST-MPAS-Official/commit/57b42fa1d7649117bbf28a69c47697ea8756d268).
It already contains both the 10 km CU cutoff and the two active
`ZT = 0.1 * Z0` assignments used by the historical-result workflow, so no
manual source edit is required.

The `ZT` behavior originates from upstream HKUST-MPAS commit
...
recalculate `Z0`. For a new independent experiment, keep the implementation
provided by the selected source version and record its branch or commit.
